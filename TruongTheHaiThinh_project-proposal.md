...
Ở giai đoạn MVP, hệ thống sẽ hoàn thiện toàn bộ luồng nghiệp vụ cốt lõi (nhóm MUST-HAVE) và giao diện web cơ bản để đảm bảo vòng đời sử dụng hoàn chỉnh từ đăng ký đến kết thúc ca khám:  
- **Xác thực & Phân quyền:** Đăng ký, đăng nhập cả 3 vai trò trên giao diện web. JWT middleware bảo vệ toàn bộ endpoint. Admin phê duyệt tài khoản bác sĩ.  
...
- **Giao diện Web:** 3 dashboard tương ứng 3 role (Patient, Doctor, Admin) bằng HTML/CSS/JS thuần, giao tiếp với backend qua Fetch API.  
- **API Docs:** Swagger UI hoàn chỉnh tại `/docs`, mọi endpoint có mô tả, schema request/response và ví dụ.  
- **Seed Data:** Script Python `seed_data.py` tạo dữ liệu mẫu (3 bác sĩ, 5 bệnh nhân, 1 admin, các chuyên khoa) cho PostgreSQL để test ngay sau khi chạy migration.
