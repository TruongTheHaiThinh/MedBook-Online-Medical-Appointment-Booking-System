...
if %errorlevel% neq 0 (
    ...
    echo Vui lòng cài đặt Python (3.9 - 3.12) và chọn "Add Python to PATH".
    pause
    exit /b 1
)

echo [1/4] Đang cài đặt các thư viện cần thiết...
...
echo [3/4] Đang khởi động Backend Server (Cổng 8000)...
start "MedBook - Backend Server" cmd /k "cd backend && set PYTHONPATH=.&& python -m uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload"

:: 4. Chạy Frontend trong cửa sổ mới
echo [4/4] Đang khởi động Frontend Server (Cổng 5500)...
start "MedBook - Frontend Server" cmd /k "python -m http.server 5500"
